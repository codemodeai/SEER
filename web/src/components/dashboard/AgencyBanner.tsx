...
export default function AgencyBanner() {
  const { agencySlug, agencyName, agencyRole } = useDashboard();

  // Don't show for owners (they already know) or if no agency
  if (!agencyName || agencyRole === "owner") return null;

  // Admin with portal access
  if (agencyRole === "admin" && agencySlug) {
    return (
      <div className="bg-gradient-to-r from-terracotta/5 to-amber-50 border border-terracotta/20 rounded-2xl px-5 py-4 flex items-center gap-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-charcoal">
            You&apos;re an admin of <strong>{agencyName}</strong>
          </p>
          <p className="text-xs text-muted mt-0.5">
            You have access to the agency portal to manage users and settings.
          </p>
        </div>
        <Link
  ...
  // Regular member — no portal access, just info
  if (agencyRole === "member") {
    return (
      <div className="bg-gradient-to-r from-terracotta/5 to-amber-50 border border-terracotta/20 rounded-2xl px-5 py-4 flex items-center gap-4">
        <div className="w-10 h-10 rounded-xl bg-terracotta/10 flex items-center justify-center flex-shrink-0">
          <Building2 size={20} className="text-terracotta" />
